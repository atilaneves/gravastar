...
CVersusMenu::CVersusMenu(CSprite *cursorSprite,
			 const CGravMenu &gravMenu):
  CStringMenu("Versus"),
  mCursor(*this, cursorSprite) {
  AddMenu(mShipMenu    = new CShipMenu(gravMenu, *this));
  CreateTypes();
...
void CVersusMenu::CreateTypes() {
  const std::string tnames[]={"Survival", "Frag Limit", "Time Limit", "Squad"};
  strings_t strings(tnames, tnames + sizeof(tnames) / sizeof(std::string));
