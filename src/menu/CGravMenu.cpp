...
CGravMenu::CGravMenu(const CGravOptions &options):
  CRootMenu(options.GetClientOptions().IsWindowed(),
	    ...
	    CFont(0),
	    cursorSprite =
	    new CSprite(CDataFile("Ships/Delta.dat").GetData(BLUE_000))),
...
CGravOptions CGravMenu::GetGravOptions() const {
  return CGravOptions(mVersusMenu->GetMeleeOptions(),
		      mOptionsMenu->GetClientOptions(mVersusMenu->GetNbPilots()));
}
