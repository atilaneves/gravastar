...
public:
    ...
    uint8_t GetSpecialHash()     const { return mSpecial->GetSpecialHash(); }
    size_t  GetSpecialStatHash() const { return mSpecial->GetStatSpriteHash(); }
    float   GetSuperGauge()      const { return mSuper.GetGauge(); }
    size_t  GetSuperStatHash()   const { return mSuper.GetSpriteHash(); }
